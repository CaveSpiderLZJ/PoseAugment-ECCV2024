...
from ..rotation2smpl import Rotation2smpl
...
class CAE(nn.Module):
    def __init__(self, encoder, decoder, device, lambdas, latent_dim, outputxyz,
        ...
        self.param2xyz = {"pose_rep": self.pose_rep, "glob_rot": self.glob_rot, "glob": self.glob,
            "jointstype": self.jointstype, "translation": self.translation, "vertstrans": self.vertstrans}
        
        # custom
        self.rotation2smpl = Rotation2smpl(device=self.device)
        self.param2smpl = {"pose_rep": self.pose_rep, "glob_rot": self.glob_rot, "glob": self.glob,
            "jointstype": self.jointstype, "translation": self.translation, "vertstrans": self.vertstrans}
    ...
    def rot2smpl(self, x, mask, **kwargs):
        kargs = self.param2smpl.copy()
        kargs.update(kwargs)
        return self.rotation2smpl(x, mask, **kargs)
    ...
    def generate(self, classes, durations, nspa=1, noise_same_action="random",
            noise_diff_action="random", fact=1):
        ...
        if False:
            ...
            print(f'\t{durations=}')
            print(f'\t{nspa=}')
            print(f'\t{noise_same_action=}')
            print(f'\t{noise_diff_action=}')
            print(f'\t{fact=}')
            
        if nspa is None: nspa = 1
        nats = len(classes)
        y = classes.to(self.device).repeat(nspa)  # (view(nspa, nats))

        if len(durations.shape) == 1:
            lengths = durations.to(self.device).repeat(nspa)
        else: lengths = durations.to(self.device).reshape(y.shape)
        # lengths.shape = [nspa * nats]
        # only useful when lengths are not the same
        mask = self.lengths_to_mask(lengths)    # [nspa * nats, max_length]
        ...
        if noise_same_action == "random":
            ...
            else: raise NotImplementedError("Noise diff action must be random or same.")
        ...
        else: raise NotImplementedError("Noise same action must be random, same or interpolate.")
