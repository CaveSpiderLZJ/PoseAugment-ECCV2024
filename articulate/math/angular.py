...
__all__ = ['RotationRepresentation', 'to_rotation_matrix', 'radian_to_degree', 'degree_to_radian', 'normalize_angle',
           ...
           'axis_angle_to_rotation_matrix', 'rotation_matrix_to_axis_angle', 'rotation_matrix_to_axis_angle2', 'r6d_to_rotation_matrix',
...
def rotation_matrix_to_axis_angle2(r: torch.Tensor):
    r"""
    Turn rotation matrices into axis-angles. (torch, batch)
    A faster implementation compared with rotation_matrix_to_axis_angle.

    :param r: Rotation matrix tensor that can reshape to [batch_size, 3, 3].
    :return: Axis-angle tensor of shape [batch_size, 3].
    """
    r = r.view(-1, 3, 3)
    theta = (0.5 * (r.diagonal(dim1=1, dim2=2).sum(dim=1) - 1)).clip(-1,1).acos()
    res = 0.5 * (r - r.transpose(1, 2)) / (theta.sin()[:,None,None] + 1e-8)
    return torch.column_stack([res[:,2,1], res[:,0,2], res[:,1,0]]) * theta[:,None]
...
def rotation_matrix_to_r6d(r: torch.Tensor):
    ...
    """
    # warning: turned clone() to contiguous().
    return r.view(-1, 3, 3)[:, :, :2].transpose(1, 2).contiguous().view(-1, 6)
