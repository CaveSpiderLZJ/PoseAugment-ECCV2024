...
class ParametricModel:
    ...
    def __init__(self, official_model_file: str, use_pose_blendshape=False, device=torch.device('cpu')):
        ...
        self.face = data['f']
        # parent is a list, describing the index of the parant of each joint,
        # like [None, 0, 0, 0, 1, 2, 3, ..., 20, 21], len = 24
    ...
    def forward_kinematics(self, pose: torch.Tensor, shape: torch.Tensor = None, tran: torch.Tensor = None,
        ...
        # j: [T, 24, 3], the joint zero positions.
        # v: [T, 6890, 3], the mesh zero positions.
        j, v = [_.expand(pose.shape[0], -1, -1) for _ in self.get_zero_pose_joint_and_vertex(shape)]
        # bone vector is the position difference between each joint and its parent joint.
        # transformation_matrix computes the homegenous transformation matrices between two different frames.
        T_local = M.transformation_matrix(pose, self.joint_position_to_bone_vector(j))
        # local transformation to global transformation.
        T_global = self.forward_kinematics_T(T_local)
        # decode to final global joint position and rotations.
        # pose_global: [T, 24, 3, 3], joint_global: [T, 24, 3]
