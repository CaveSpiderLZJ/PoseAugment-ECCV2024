...
class GaussianDiffusion:
    ...
    def p_sample(
        ...
        # out: dict, mean, variance, log_variance, pred_xstart -> tensor with the same shape as x
        noise = th.randn_like(x)
        if const_noise: # const_noise == False
            noise = noise[[0]].repeat(x.shape[0], 1, 1, 1)

        nonzero_mask = ((t != 0).float().view(-1, *([1] * (len(x.shape) - 1))))  # no noise when t == 0
        if cond_fn is not None:     # cond_fn is None
            out["mean"] = self.condition_mean(cond_fn, out, x, t, model_kwargs=model_kwargs)
        # log_variance = log(sigma^2), exp(0.5 * log_variance) = sigma
        mean_sigma = th.mean(th.exp(0.5 * out["log_variance"]))
    ...
    def p_sample_loop(
        ...
        Customized to generate motions in M2M manner.
        ...
        """
        final = None
        if dump_steps is not None:  # dump_steps == None
            dump = []
        ...
        for i, sample in enumerate(self.p_sample_loop_progressive(
            ...
            if dump_steps is not None and i in dump_steps:  # never entered
    ...
    def p_sample_loop_progressive(
        ...
        Cutomized to generate motions in M2M manner.
        ...
        if noise is not None:   # noise is None
            img = noise
        else: img = th.randn(*shape, device=device) # [N, C, H, W], random noise

        if skip_timesteps and init_image is None:   # not entered
            init_image = th.zeros_like(img)
        # init_image == None

        indices = list(range(self.num_timesteps - skip_timesteps))[::-1]
        # indices from n_step - 1 to 0
        ...
        for i in indices:
            ...
            # list of t, len == batch_size
            # randomize_class == False
            if randomize_class and 'y' in model_kwargs:
                model_kwargs['y'] = th.randint(low=0, high=model.num_classes,
                    size=model_kwargs['y'].shape, device=model_kwargs['y'].device)
                
            with th.no_grad():
                # cond_fn_with_grad == False
                sample_fn = self.p_sample_with_grad if cond_fn_with_grad else self.p_sample
                # simulate M2M pose generation
                if t_expand is not None and i == t_expand:  # expand input, to generate M2M motions
                    img = img.repeat(n_expand, 1, 1, 1)
                    t = t.repeat(n_expand)
                    model_kwargs['y']['mask'] = model_kwargs['y']['mask'].repeat(n_expand, 1, 1, 1)
                    model_kwargs['y']['lengths'] = model_kwargs['y']['lengths'].repeat(n_expand)
                    model_kwargs['y']['text'] = model_kwargs['y']['text'] * n_expand
                    model_kwargs['y']['tokens'] = model_kwargs['y']['tokens'] * n_expand
                    model_kwargs['y']['scale'] = model_kwargs['y']['scale'].repeat(n_expand)
                    shape = (shape[0] * n_expand, shape[1], shape[2], shape[3])
                out = sample_fn(model, img, t, clip_denoised=clip_denoised, denoised_fn=denoised_fn,
                    cond_fn=cond_fn, model_kwargs=model_kwargs, const_noise=const_noise)
                yield out
                img = out["sample"]
